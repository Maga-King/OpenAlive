...
final class CosmicRenderTest {
    ...
    static void run(Context context)throws Exception{
        ...
        require(EGL14.eglChooseConfig(display,attributes,0,configs,0,1,count,0)&&count[0]>0,"GLES3 unavailable");
        ...
        try{
            ...
            for(int i=0;i<entries.length();i++)for(boolean dark:new boolean[]{false,true}){
                ...
                try(CosmicRenderer renderer=new CosmicRenderer(context.getAssets(),variant,dark,width,height,0)){
                    ...
                    for(int scene:new int[]{0,1,2,0,2,1}){
                        ...
                        if(variant==9&&!dark){
                            ...
                            try(OutputStream out=new FileOutputStream(new File(directory,"space-"+scene+".png"))){bitmap.compress(Bitmap.CompressFormat.PNG,100,out);}finally{bitmap.recycle();}
                        }
                    }
                }
            }
