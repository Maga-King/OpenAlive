...
sources=[root/'app/java/org/aliveclean'/f'{name}.java' for name in ['AodRegion','SceneState','AodMotionWindow','AodTransitionWindow','ContinuousAodPolicy']]+list((root/'tests/host').rglob('*.java'))
subprocess.run([str(jdk/('javac'+exe)),'-encoding','UTF-8','-d',str(out),*map(str,sources)],check=True)
for name in ['AodRegionTest','SceneStateTest','AodMotionWindowTest','AodTransitionWindowTest','ContinuousAodPolicyTest']:
